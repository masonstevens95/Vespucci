...
describe("GoldFlowSection", () => {
  ...
  it("does not show Monthly Gold Expense when monthlyGoldExpense = 0", () => {
    const { container } = render(<GoldFlowSection stats={mkStats({ monthlyGoldExpense: 0 })} />);
    expect(container.textContent).not.toContain("Monthly Gold Expense");
  });

  it("shows divider and Inflation row when inflation != 0", () => {
    const { container } = render(<GoldFlowSection stats={mkStats({ inflation: 500 })} />);
  ...
  it("does not show Inflation when inflation = 0", () => {
    const { container } = render(<GoldFlowSection stats={mkStats({ inflation: 0 })} />);
