...
describe("MilitaryTab", () => {
  it("renders army and navy sections together", () => {
  ...
  it("wraps content in modal-rows div", () => {
    const { container } = render(<MilitaryTab stats={mkStats()} />);
    expect(container.querySelector(".modal-rows")).not.toBeNull();
  });

  it("shows full navy stats when ships present", () => {
    const { container } = render(<MilitaryTab stats={mkStats({ heavyShips: 5 })} />);
