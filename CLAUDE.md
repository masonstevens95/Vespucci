...
- **A lake inside a location is a hole in its polygon, and holes are not coastline.** The coast pass draws whatever touches no land, so an interior ring — which by definition has no land on the far side — got stroked as a closed outline around the lake, in the middle of somebody's territory. 1,896 rings in the asset are shaped that way. `coastline` now skips a ring that lies inside another ring of the same shape *and* touches no neighbour. The touch test is what separates a lake from an **enclave** (one country wholly inside another, 219 of them), whose edges are a border and must stay
- **Containment is decided by ray casting, not by bounding boxes.** A crescent-shaped location's box contains the bay it wraps around, so an island sitting in that bay would be called a hole and silently lose its coastline. The box test is kept only as a cheap pre-filter before the ray cast
- Only lakes *wholly inside one location* are caught. A lake lying between several locations (Mälaren, Vänern, Ladoga, the Great Lakes) is a gap between them rather than a ring of any one of them, and nothing local tells it apart from open sea — that would need a flood fill of the map's empty space from the edge
...
- **A wasteland's claim reaches exactly one step.** It is judged on every ordinary location that it — or any wasteland touching it — touches. Judging on its own neighbours alone leaves chain interiors grey, since their only neighbours are other wastelands. Judging a whole connected component at once goes too far: the asset chains deserts together with corridor shapes (`Connector_Egypt5`, `Connector_Libya16`, `Horn_Desert_Corridor3`), which put the entire Sahara in one component of 125 shapes bordered by 21 countries. Egypt's own deserts sat in a component bordered by the Mamluks on 42 of 49 locations and stayed grey because of the other seven — a veto a continent away is not a fact about the shape being painted
- **One step is the smallest reach that keeps touching wastelands in agreement.** If A and B touch and both are claimed, A's reach covers B's ordinary neighbours, so they all carry A's tag; B being claimed means they all carry B's tag; so the tags match. At reach zero that argument fails and both sample saves produce a dozen places where two touching deserts take different colours — a boundary the save never asserts. Do not lower it
